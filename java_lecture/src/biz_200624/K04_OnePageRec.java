...
class K04_Record {
	...
	static int k04_iPerson = 30; // 사람 수
	...
	public static void main(String[] args) {
		...
		if (K04_Record.k04_iPerson % k04_OnePage == 0) { // 나눠서 딱떨어지면
			for (int k04_i = 0; k04_i < (K04_Record.k04_iPerson / k04_OnePage); k04_i++) {
				...
				K04_Record.k04_dataSet(); // 데이터를 어레이에 저장하는 메서드
				...
				for (int k04_j = 0 + (k04_OnePage * k04_i); k04_j < k04_OnePage + (k04_OnePage * (k04_i)); k04_j++) {
					...
					if (k04_j == K04_Record.k04_iPerson) {// 만약 j값이 사람수랑 일치하면
						break; // 나감
					}
					K04_Record.k04_ItemPrint(k04_j); // 성적 출력
				}
				K04_Record.k04_TailPrintNow();// 현재페이지 출력
				K04_Record.k04_TailPrint(); // 누적페이지 출력
			}
		} else { // 나눠서 딱 떨어지지 않으면
