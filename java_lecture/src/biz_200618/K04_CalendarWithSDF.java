...
public class K04_CalendarWithSDF {
	public static void main(String[] args) {
		...
		/////////////////////////////////////////////
		// # 돈은 영어 기준으로 세자리마다 콤마를 찍는다
		//
		// DecimalFormat이라는 클래스를 가져다 쓴다.
		DecimalFormat k04_df = new DecimalFormat("###,###,###,###,###");
		System.out.printf("***************************************************************************************\n");
		System.out.printf("*                                콤마찍기, 날짜 적용                                  *\n");
		System.out.printf("총 수수료 : %s원 => 미화 : %s달러, 달러당 수수료 : %f원\n", k04_df.format(k04_i_TotalCom),
				k04_df.format(k04_usd), k04_ComPerOne); // 깔끔
		...
		System.out.printf("총 한화환전금액 : %s원 => 미화 : %s달러, 수수료 징수 : %s원,  잔돈 : %s원\n", k04_df.format(k04_MyWon),
				k04_df.format(k04_usd), k04_df.format(k04_i_TotalCom), k04_df.format(k04_remain)); // easy
		//////////////////////////////////////////////
		// 처음에는 베껴쓰고 점점 암기(시스템의 현재 시간 가지고 오기, 3줄을 가져다 쓰기
		Calendar k04_calt = Calendar.getInstance(); // 캘린더 인스턴스를 생성한다
		SimpleDateFormat k04_sdt = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss"); // sdf 인스턴스를 생성 후 날짜시간의 모양을 지정한다
		System.out.printf("최종실행시간 : %s\n", k04_sdt.format(k04_calt.getTime())); // 캘린더의 값을 받아 sdf형식으로 포맷하여 출력
		System.out.printf("***************************************************************************************\n");
